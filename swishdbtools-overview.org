...
* Introduction
** DESCRIPTION
*** DEPRECATED DESCRIPTION-code NO TANGLE
#+name:DESCRIPTION
...
** data_inventory_etl
*** R-data_inventory_etl
#+name:data_inventory_etl
#+begin_src R :session *R* :tangle R/data_inventory_etl.r :exports none :eval no
################################################################
# name:data_inventory_etl

#+end_src
*** test-data_inventory_etl
#+name:data_inventory_etl
#+begin_src R :session *R* :tangle tests/test-data_inventory_etl.r :exports none :eval no
################################################################
# name:data_inventory_etl
...
#+end_src
** COMMENT DEPRECATED, MOVED TO RTAMET sql_tsfill
*** R-sql_tsfill
#+name:sql_tsfill
#+begin_src R :session *R* :tangle R/sql_tsfill.r :exports none :eval no
  ...
  sql_tsfill <- function(tableIn, variableName, fillValue, ...)
  {
    #argumentNames <- c(deparse(substitute(variable1)), sapply(substitute(list(...))[-1], deparse))
    #argumentNames <- paste(collapse = ", ", argumentNames)
    ...
    for(variableIndex in 1:length(otherVariables)) 
    {
        if(variableIndex == 1) {
            variable <- otherVariables[variableIndex]
            argumentNames <- paste(sep = "", names(variable))
        } else {
            variable <- otherVariables[variableIndex]
            argumentNames <- paste(sep="", argumentNames, ", ", names(variable))
        }
    }
    ...
    joinExpression <- paste(sep = "", "(select ", argumentNames, " from ",tableName,") ", "\r\n", "t1 left join ", tableName, " t2 on ", "\r\n", categoryExpression)
...
#+begin_src R :session *R* :tangle tests/test-sql_tsfill.r :exports none :eval no
  ...
  sparseTable <- "postcode, age, date, value
  A, X, 1, 10
  A, Z, 2, 20
  B, Y, 3, 10
  "
  ...
  ## variable2 <- as.data.frame(c("X", "Y", "Z"))
  ## names(variable2) <- 'factorB'
  ## tableIn

  ## debug(sql_tsfill)
  ## filledTable <- sql_tsfill(
  ##     tableIn = tableIn
  ##    ,
  ##     variableName = "value"
  ##    ,
  ...
  tableIn
  library(data.table)
  d1 <- data.table(tableIn)
  byvars <- names(tableIn)[-ncol(tableIn)]
  d1
  merge(tableIn[,"postcode"], tableIn[,"age"], tableIn[,"date"], all.x = T)

  #d2 <- data.table(expand.grid(unique(tableIn$factorA), unique(tableIn$factorB)))
  res1 <- do.call(cbind, tableIn
                  lapply(seq_along(v1, function(i) combn(v1, i)))
  df.1 <- tableIn                
  ##find all (unique) state-district-county combinations
  ##df.2 <- unique(df.1[,c("postcode","age", "date")])
  df.2 <- list(postcode = unique(tableIn$postcode), age = unique(tableIn$age), date = unique(tableIn$date))
  df.2
  a <- cross_df(df.2)

  a
  a[a$postcode == "B",]
  merge(a, tableIn, all.x = T)

  #find all (unique) years
  df.3 <- unique(df.1[,"date",drop=FALSE])
  ...
  d2
  setDT(d2)
  setnames(d2, names(d2), byvars)
  d3 <- merge(d2, tableIn, by = byvars, all.x = T)
...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  library(purrr)
  tableIn <- read.csv(textConnection("postcode, age, date, value
  A, X, 1, 10
  A, Z, 2, 20
  B, Y, 3, 10
  "))
  df2 <- list(postcode = unique(tableIn$postcode),
               age = unique(tableIn$age),
               date = unique(tableIn$date))
  df3 <- cross_df(df2)
  df4 <- merge(df3, tableIn, all.x = T)
  df4[is.na(df4$value),"value"] <- 0
  df4
  "
     postcode age date value
  1         A   X    1    10
  2         A   X    2     0
  3         A   X    3     0
  4         A   Y    1     0
...
#+end_src
* vignettes
*** COMMENT make_ddixml-code
#+name:make_ddixml
#+begin_src tex :tangle inst/doc/make_ddixml.Rnw :eval no :padline no
