...
#+begin_src R :session *R* :tangle R/sql_subset.r :exports none :eval no
  ...
  # TODO add argument to ignore_geom ie:
    ## file_i <- pgListTables(ewedb, "drought_nswdpi")[1,]
    ## file1 <- sql_subset(ewedb, paste("drought_nswdpi",file_i, sep ="."), limit = 1, eval = T)
    ## goodCols <- names(file1)[-grep('the_geom', names(file1))]
    ## file1 <- sql_subset(ewedb, paste("drought_nswdpi",file_i, sep ="."),
    ##                     limit = 10, eval = T, select =
    ##                     paste(goodCols, sep = "", collapse = ",")
    ##                     )
